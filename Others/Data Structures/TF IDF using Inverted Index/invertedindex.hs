...
inverted doc=do
	...
	ls <- forM files (\a -> do	--apply the function to every file
		...
		let purify = filter (not . null )[[x | x<-b, x `elem` ['a'..'z'] || x `elem` ['A'..'Z']] | b<-words contents] --only keep pure words i.e no punctuation and special characters
...
toTry query = do
	...
	if (length result == 0) then print $ "Nothing Found" else T.writeFile "searchresult.html" (renderHtml (draw query [(fst x,snd x)| x<-result])) --result is printed if found
	--result is in the format of ("document: tfidf score")

draw direc xs = B.html $ do
    B.head $ B.title (B.text (D.pack "Search Result"))
    B.body $ do
        B.h1  (B.text (D.pack ("Showing results for '" ++unwords direc ++ "'")))
        B.table $ forM_ xs $ \(f,l) ->
            B.tr $ do
                B.td (B.a B.! href (B.stringValue ("archive/" ++ f)) $ (B.text (D.pack ([(toUpper (head f))] ++ (tail f))))) 
